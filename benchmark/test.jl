...
function run_benchmark(instances_folder::String)
    ...
    n_total   = length(files) * n_configs * length(methods)
    counter   = 0
    rows      = []
    ...
    println("="^70)
    println("BENCHMARK: $(length(files)) instances × $n_configs configs × $(length(methods)) methods = $n_total runs")
    println("Time limit: 1000s per run")
    println("="^70 * "\n")
        ...
        for p in p_values
            ...
            for fpath in files
                ...
                for (method_name, solver) in methods
                    counter += 1
                    print("  [$counter/$n_total] $fname | $method_name ... ")
                    flush(stdout)
