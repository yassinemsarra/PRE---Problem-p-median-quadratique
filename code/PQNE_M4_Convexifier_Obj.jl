...
function main()
    ...
    for (n_clients, n_sites, p) in instances
        ...
        # Résolution
        relax, obj, bnd, gp, nds, t = solve_p_median_quadratic_convex_obj(n_clients, n_sites, p, d, f, Q)

        # Enregistrement
        push!(results, ("Convexification objectif", n_clients, n_sites, p, relax, obj, bnd, gp, nds, t))
    end

    # Exportation CSV unique pour ce fichier
    mkpath(joinpath(@__DIR__, "..", "results", "individual"))
    filename = joinpath(@__DIR__, "..", "results", "individual", "PQNE_M4_" * Dates.format(now(), "yyyymmdd") * ".csv")
