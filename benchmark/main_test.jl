...
function main()
    ...
    n_sites   = 60 #inst.n
    n_clients = 60 #inst.m
    p         = 15 #max(1, round(Int, 0.1 * n_sites))
    f         = Float64.(inst.c)
    d         = Float64.(inst.d')  
    Q         = Float64.(inst.a)
    n = 60
    ...
    # M1 : solve_p_median_quadratic_gurobisolve_p_median_quadratic_gurobi(n_clients, n_sites, p, d, f, Q, prelinearize_val)
    ...
    val_relax, obj, bound, gap, nodes, t_solve = solve_p_median_quadratic_gurobi(n_clients, n_sites, p, d[1:n, 1:n], f, Q[1:n, 1:n], 2)
